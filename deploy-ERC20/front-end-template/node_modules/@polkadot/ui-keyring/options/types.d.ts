import { KeyringItemType, KeyringStruct } from '@polkadot/ui-keyring/types';
export interface KeyringSectionOption {
    key: string | null;
    name: string;
    value: string | null;
}
export declare type KeyringSectionOptions = KeyringSectionOption[];
export declare type KeyringOptions = {
    [type in KeyringItemType | 'all' | 'allPlus' | 'recent' | 'testing']: KeyringSectionOptions;
};
export declare type KeyringOption$Type = keyof KeyringOptions;
export interface KeyringOptionInstance {
    createOptionHeader: (name: string) => KeyringSectionOption;
    init: (keyring: KeyringStruct) => void;
}
