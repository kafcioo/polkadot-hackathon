...
async function query(api, keyring) {
  ...
  const multiUnsub = await api.queryMulti([[api.query.system.account, keyring.eve.address], // older chains only
  [api.query.system.accountNonce, keyring.bob.address]], values => {
    console.log('values', values);
    multiUnsub();
  }); // check multi , Promise result

  const multiRes = await api.queryMulti([[api.query.system.account, keyring.eve.address], // older chains only
  [api.query.system.accountNonce, keyring.bob.address]]);
  console.log(multiRes); // events destructing
  ...
  await api.query.balances.freeBalance.range(['0x1234'], keyring.bob.address); // check range types
...
function types(api) {
  ...
  const gasUnsafe = (0, _types.createTypeUnsafe)(registry, 'Gas', [2]);
  const overriddenUnsafe = (0, _types.createTypeUnsafe)(registry, 'Gas', [2]);
...
async function main() {
  ...
  const keyring = (0, _testingPairs.default)(); // eslint-disable-next-line @typescript-eslint/no-floating-promises

  Promise.all([consts(api), derive(api), query(api, keyring), rpc(api), types(api), tx(api, keyring)]);
